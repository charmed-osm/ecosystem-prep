...
class GraylogCharm(CharmBase):
    ...
    def _on_mongodb_relation_changed(self, event):
        ...
        if mongodb_uri is None or mongodb_rs_name is None:
            logger.warning(
                "No replica_set_uri or replica_set_name in MongoDB relation data"
            )
            return
        # if we have the data we need, get the information
        self._stored.mongodb_uri = "{}graylog?replicaSet={}".format(
            mongodb_uri, mongodb_rs_name
        )
            ...
            "containers": [
                {
                    ...
                    "ports": [
                        ...
                        {"name": "beats", "containerPort": 5044, "protocol": "TCP"},
                    ],
                    "envConfig": {
    ...
    def _configure_pod(self):
        ...
        if not self.has_mongodb or not self.has_elasticsearch:
            ...
            self.unit.status = BlockedStatus(
                "Need mongodb and Elasticsearch relations."
            )
            return
